...
class Ui_RegisterUser(object):
    ...
    def registerButtonPress(self):
        ...
        if res is "ok":
            Session = get_session_maker()
            session = Session()
            user = User(self.loginId.text(),self.password.text(),self.email.text(),self.loginId_2.text(),self.loginId_3.text(),self.loginId_4.text(),self.loginId_5.text(),self.loginId_6.text())
            session.add(user)
            session.commit()
            session.close()
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label.setText(_translate("MainWindow", "<html><head/><body><p><span style=\" font-size:24pt; font-weight:600;\">User Registration</span></p></body></html>"))
